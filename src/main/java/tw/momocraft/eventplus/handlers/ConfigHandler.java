package tw.momocraft.eventplus.handlers;
...
import tw.momocraft.eventplus.Commands;
import tw.momocraft.eventplus.utils.DependAPI;
import tw.momocraft.eventplus.utils.Logger;
...
public class ConfigHandler {
    ...
    public static void generateData(boolean reload) {
        ...
        if (!reload && getConfig("config.yml").getBoolean("Clean.Settings.Auto-Clean.Enable")) {
            ...
            }.runTaskLater(tw.momocraft.eventplus.EventPlus.getInstance(), delay);
        } /* else if (!reload && getConfig("config.yml").getBoolean("Clean.Settings.Schedule.Enable")) {
            BukkitScheduler scheduler = getServer().getScheduler();
            scheduler.scheduleSyncDelayedTask(EventPlus.getInstance(),
                    new BukkitRunnable() {
                        @Override
                        public void run() {
    ...
    public static void registerEvents() {
        tw.momocraft.eventplus.EventPlus.getInstance().getCommand("eventplus").setExecutor(new Commands());
    }

    public static FileConfiguration getConfig(String path) {
        File file = new File(tw.momocraft.eventplus.EventPlus.getInstance().getDataFolder(), path);
        if (configYAML == null) {
            getConfigData(path);
        }
    ...
    private static FileConfiguration getConfigData(String path) {
        ...
        if (!(file).exists()) {
            try {
                tw.momocraft.eventplus.EventPlus.getInstance().saveResource(path, false);
            } catch (Exception e) {
                tw.momocraft.eventplus.EventPlus.getInstance().getLogger().warning("Cannot save " + path + " to disk!");
                return null;
            }
        }
    ...
    private static void configFile() {
        ...
        File File = new File(tw.momocraft.eventplus.EventPlus.getInstance().getDataFolder(), "config.yml");
        if (File.exists() && getConfig("config.yml").getInt("Config-Version") != 1) {
            if (tw.momocraft.eventplus.EventPlus.getInstance().getResource("config.yml") != null) {
                LocalDateTime currentDate = LocalDateTime.now();
                DateTimeFormatter formatter = DateTimeFormatter.ofPattern("yyyy-MM-dd HH-mm-ss");
                String currentTime = currentDate.format(formatter);
                String newGen = "config " + currentTime + ".yml";
                File newFile = new File(tw.momocraft.eventplus.EventPlus.getInstance().getDataFolder(), newGen);
                if (!newFile.exists()) {
                    File.renameTo(newFile);
                    File configFile = new File(tw.momocraft.eventplus.EventPlus.getInstance().getDataFolder(), "config.yml");
                    configFile.delete();
                    getConfigData("config.yml");
